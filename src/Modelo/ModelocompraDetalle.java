...
*/
    static public List<compraDetalle> MostrarcompraDetalle() {
        ...
        try {
            ...
            ps = con.prepareStatement("select m.idMedida, m.descripcion, m.siglas, "
                    ...
                    +"p.idProveedor, p.razonSocial, p.tipoPersona, p.nrodocumento, p.direccion , p.telefono, "
                    + "d.idDocumento, d.descripcion, d.siglas, "
                    + "c.idCompra , c.serieDocumento  , c.nroDocumento  ,c.fecha, "
                    +"cd.idCompraDetalle, cd.cantidad"
                    + " from compra c, articulo a, compraDetalle cd, proveedor p, documento d, medida m, medida m2"
                    + " where cd.idCompra = c.idCompra and cd.idArticulo = a.idArticulo and m.idMedida= a.idUnidadMedidaCompra and m2.idMedida= a.idUnidadMedidaVenta and c.idDocumento=d.idDocumento and p.idProveedor=c.idProveedor");
            rs = ps.executeQuery(); 
            int i = 0;
            while (rs.next()) {
                ...
                p = new Proveedor(idProveedor, razonSocial, tipoPersona, nrodocumento, direccion, telefono);
                ...
                c = new Compra(idCompra, d, serieDocumentoc, nroDocumentoc, fecha, p);
                
                int idCompraDetalle = rs.getInt(28);
                double cantidad = rs.getDouble(29);

                compraDetalle cd = new compraDetalle(idCompraDetalle, c, a, cantidad);
    ...
    static public boolean RegistracompraDetalle(compraDetalle compraD) {
        boolean band = false;
        Connection con = Conexion.getConexion();
        try {
            ps = con.prepareStatement("insert into compraDetalle(idCompraDetalle, idCompra, idArticulo, cantidad) values\n"
                    + "(?,?,?,?);");

            ps.setInt(1, (int) compraD.getIdCompraDetalle());
            ps.setInt(2, (int) compraD.getIdCompra().getIdCompra());
    ...
    static public boolean EliminacompraDetalle(int fila, String idcompraDetalle) {
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("Delete from compraDetalle where idcompraDetalle=" + idcompraDetalle + ";");
            ps.executeUpdate();
